...
const defaultTags: TagItem[] = [
  { id: "tag-goals", name: "Goals", color: "#9C9F84" }, // Sage
  { id: "tag-assignments", name: "Assignments", color: "#000080" }, // Navy
  { id: "tag-appointments", name: "Appointments", color: "#d97706" }, // Amber/Orange
  { id: "tag-projects", name: "Projects", color: "#7c3aed" }, // Violet
];

// Demo defaults
const initialBank: MasterTask[] = [
  { id: "m1", text: "Read Chapter 4", is_priority: false, tag_id: "tag-assignments" },
  { id: "m2", text: "Math Homework", is_priority: false, tag_id: "tag-assignments" },
  { id: "m3", text: "Finish Essay Draft", is_priority: true, tag_id: "tag-assignments" },
  { id: "m4", text: "Dentist", is_priority: true, tag_id: "tag-appointments" },
  { id: "m5", text: "Learn Next.js", is_priority: false, tag_id: "tag-goals" },
  { id: "m6", text: "Start SaaS Project", is_priority: true, tag_id: "tag-projects" }
];
...
export default function Home() {
  ...
  const [taskBank, setTaskBank] = useState<MasterTask[]>(initialBank);
  ...
  // Tag Palette State inside Modal
  ...
  // Sync default modal date to view if opened
  useEffect(() => {
    if (!isModalOpen) setNewTaskDate(dateKey);
  }, [dateKey, isModalOpen]);

  // Handle focus for meal input
  useEffect(() => {
    if (activeMealInput && mealInputRef.current) {
      // Use a small timeout to ensure it focuses after the render cycle completes
      setTimeout(() => mealInputRef.current?.focus(), 0);
    }
  }, [activeMealInput]);

  // Set temp color appropriately when creating tag vs modifying
  useEffect(() => {
  ...
  // Initialize store with "today" having some schedule from initial bank
  const [dataStore, setDataStore] = useState<TaskData>(() => {
    const todayStr = getDateKey(new Date());
    const initialItems: TaskItem[] = [
      { id: "t1", master_id: "m1", text: "Read Chapter 4", is_done: false, is_priority: false, tag_id: "tag-assignments" },
      { id: "p1", master_id: "m4", text: "Dentist", is_done: false, is_priority: true, tag_id: "tag-appointments" },
      { id: "g1", master_id: "m5", text: "Learn Next.js", is_done: false, is_priority: false, tag_id: "tag-goals" },
    ];
    // Example initial meals
    const initialMeals: MealEntry[] = [
      { id: "meal1", type: "B", text: "Oatmeal with Berries" }
    ];
    return {
      [todayStr]: { items: initialItems, meals: initialMeals, water: 3, steps: "4500", step_goal: "10000", notes: "" }
    };
  });
  ...
  const handleNotesKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (e.key === 'Tab') {
      ...
      // Look for any list prefix: bullets, open brackets, numbers
      const listMatch = currentLine.match(/^(\s*)(•\s+|-\s+|\[\s\]\s+|\[x\]\s+|\d+\.\s+|[a-zA-Z]\.\s+)/);

      if (listMatch) {
        // Indent it
        const prefixAndIndent = listMatch[0];
        const textAfterPrefix = currentLine.substring(prefixAndIndent.length);
        let newPrefix = "";

        if (e.shiftKey) {
          // Outdent
          if (listMatch[1].length >= 2) {
            // Remove 2 spaces and maybe change character down
            const newIndent = listMatch[1].substring(2);
            // Simple heuristic: just remove spaces, keep the same char for now
            newPrefix = newIndent + listMatch[2];
          } else {
            newPrefix = listMatch[2]; // No indent left
          }
        } else {
          // Indent
          const newIndent = listMatch[1] + "  ";
          // Simple heuristic to change bullet style if bullet
          let char = listMatch[2];
          if (char.trim() === "•") char = "- ";
          else if (char.trim() === "-") char = "• ";
          else if (char.trim() === "1.") char = "a. ";

          newPrefix = newIndent + char;
        }

        const newText = value.substring(0, selectionStart - currentLine.length) + newPrefix + textAfterPrefix + value.substring(selectionStart);
        setDayNotes(newText);

        const newCursorPos = selectionStart - currentLine.length + newPrefix.length + textAfterPrefix.length;
        setTimeout(() => {
    ...
    if (e.key === 'Enter') {
      ...
      // Match bullets, checkboxes, or numbers
      const formatMatch = currentLine.match(/^(\s*)(•\s+|-\s+|\[\s\]\s+|\[x\]\s+|\d+\.\s+|[a-zA-Z]\.\s+)/);

      if (formatMatch) {
        e.preventDefault();
        const prefix = formatMatch[0]; // e.g. "• ", "  [ ] ", "1. "
        const textAfterPrefix = currentLine.substring(prefix.length).trim();

        // If the line was empty, delete the formatting
        if (textAfterPrefix === '') {
          const newText = value.substring(0, selectionStart - prefix.length) + value.substring(selectionStart);
          setDayNotes(newText);
        ...
        const alphaMatch = prefix.match(/(\s*)([a-zA-Z])\.\s+/);
        if (alphaMatch) {
          const nextChar = String.fromCharCode(alphaMatch[2].charCodeAt(0) + 1);
          nextPrefix = `${alphaMatch[1]}${nextChar}. `;
        }

        const newText = value.substring(0, selectionStart) + '\n' + nextPrefix + value.substring(selectionStart);
        setDayNotes(newText);
  ...
  const insertNoteFormatting = (prefix: string) => {
    ...
    if (hasFormatting) {
      // Just append at current cursor if they clicked it while a format exists, or replace it if we want to get fancy.
      // For simplicity, just insert at start of line if not formatted, or at cursor if empty
  ...
  const handleUpdateTagColor = (colorHex: string) => {
    ...
    if (isCreatingTag) return; // Wait to save when form submits

    // It's an existing tag, so update it globally
    if (newTaskTagId) {
  ...
  const scheduleTaskToDay = (masterTask: MasterTask, targetDateKey: string) => {
    ...
    const newScheduledTask: TaskItem = {
      ...masterTask,
      id: newDayId,
    ...
    setDataStore((prevStore) => {
      ...
      return {
        ...prevStore,
        [targetDateKey]: { ...existingDay, items: [...existingDay.items, newScheduledTask] }
      };
    });
  };

  const archiveMasterTask = (masterId: string) => {
    // 1. Remove from Task Bank
    setTaskBank((prev) => prev.filter((t) => t.id !== masterId));

    // 2. Automatically mark all scheduled instances of this task as "Done" globally
    setDataStore((prevStore) => {
      const newStore = { ...prevStore };
      Object.keys(newStore).forEach(date => {
        const day = newStore[date];
        const updatedItems = day.items.map(item =>
          item.master_id === masterId ? { ...item, is_done: true } : item
        );
        newStore[date] = { ...day, items: updatedItems };
      });
      return newStore;
    });
    ...
    confetti({
      ...
      spread: 70,
      origin: { y: 0.6 },
      colors: ['#000080', '#9C9F84', '#d97706', '#7c3aed'] // Theme colors
    });
  };
  ...
  // Save the currently active meal input
  const saveActiveMeal = (type: MealType, value: string) => {
    if (!value.trim()) return; // Don't save empty strings, use removeMeal if they cleared it

    const newMealText = value.trim();

    // Save to DB (DataStore)
    setDataStore(prev => {
      const existingDay = prev[dateKey] || getEmptyDay();
      const newMeals = existingDay.meals.filter(m => m.type !== type);
      newMeals.push({ id: `meal_${Date.now()}`, type: type, text: newMealText });
  ...
  const handleMealSubmit = (e: React.FormEvent | React.KeyboardEvent, type: MealType) => {
    // If not an Enter press keyboard event, exit (form submit shouldn't trigger here, but just in case)
  ...
  const handleMealBlur = (e: React.FocusEvent<HTMLInputElement>, type: MealType) => {
    // Delay slightly so if they clicked a dropdown suggestion or tabbed to next item, it registers
    setTimeout(() => {
      // Only trigger blur logic if another element within the meal block wasn't focused
      // (like clicking the remove button or the suggestion dropdown)
      // If we simply clicked elsewhere or tabbed out, save and close.
      if (activeMealInput === type) {
        if (mealInputValue.trim() !== "") {
          saveActiveMeal(type, mealInputValue);
        } else if (getMealText(type) && mealInputValue.trim() === "") {
          // They cleared the input, let's remove it
          removeMeal(type);
        }
        setActiveMealInput(null);
        setMealInputValue("");
  ...
  const handleMealKeyDown = (e: React.KeyboardEvent<HTMLInputElement | HTMLDivElement>, type: MealType) => {
    if (e.key === 'Tab') {
      ...
      if (activeMealInput === type && mealInputValue.trim()) {
        saveActiveMeal(type, mealInputValue);
      }

      if (nextType) {
        setActiveMealInput(nextType);
        setMealInputValue(getMealText(nextType));
      } else {
        // Close if tabbed past ends
        setActiveMealInput(null);
        setMealInputValue("");
      }
    }
  ...
  const goalsCompleted = goalsArray.filter((g) => g.is_done).length; // New Gold Goal tracker
  ...
  const getTabIndex = (type: MealType) => {
    ...
    switch (type) {
      ...
      case "L": return base + 2;
      case "D": return base + 3;
      case "S": return base + 4;
      default: return base;
    }
  };
      ...
      <div className="flex flex-col gap-1 relative group">
        ...
        {isActive ? (
          <div className="relative z-10 -m-1 p-1 bg-white dark:bg-zinc-900 ring-2 ring-brand-sage dark:ring-brand-sage/50 rounded-md transition-shadow">
            <div className="flex items-center gap-2 border-b border-brand-sage pb-0.5">
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Tab') handleMealKeyDown(e, type);
                  else handleMealSubmit(e, type);
                }}
                ...
                {mealSuggestions.map((suggestion, idx) => (
                  <button
                    ...
                    onMouseDown={(e) => {
                      ...
                      e.preventDefault();
                      saveActiveMeal(type, suggestion);
                      setActiveMealInput(null);
                      setMealInputValue("");
                    }}
                  >
          ...
          <div
            ...
            onClick={() => {
              setActiveMealInput(type);
              setMealInputValue(currentText);
            }}
            onFocus={() => {
              if (activeMealInput !== type) {
                setActiveMealInput(type);
  ...
  return (
    <div className="flex min-h-screen justify-center bg-zinc-100 font-sans dark:bg-black p-4 sm:p-8 relative overflow-hidden">

      {/* Master Task Bank Slide-Out */}
      <div className={cn(
        "fixed top-0 right-0 h-full w-[400px] bg-white dark:bg-zinc-950 shadow-2xl border-l border-zinc-200 dark:border-zinc-800 transition-transform duration-300 z-50 flex flex-col pl-safe pb-safe",
        isBankOpen ? "translate-x-0" : "translate-x-full"
      )}>
        <div className="flex flex-col gap-3 p-5 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 drop-shadow-sm z-10 w-full shrink-0">
          <div className="flex items-center justify-between text-brand-navy dark:text-zinc-200">
            ...
            <button onClick={() => setIsBankOpen(false)} className="p-2 text-zinc-500 hover:bg-zinc-200 dark:hover:bg-zinc-800 rounded-full transition-colors">
              <X className="w-5 h-5" />
            </button>
          </div>
          <div className="flex gap-2 items-center">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg pl-9 pr-3 py-1.5 focus:outline-none focus:border-brand-navy/50 transition-colors text-sm outline-none"
              />
            </div>
            <select
              value={bankFilterTagId || ""}
              onChange={e => setBankFilterTagId(e.target.value)}
              className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg px-2 py-1.5 focus:outline-none focus:border-brand-navy/50 transition-colors text-sm text-zinc-700 dark:text-zinc-300 min-w-[100px] outline-none"
            >
                  ...
                  <div className="flex items-center gap-2 border-b-2 border-zinc-100 dark:border-zinc-800/50 pb-2">
                    {tag ? (
                      <>
                        ...
                        <h3 className="text-sm font-bold uppercase tracking-widest text-zinc-700 dark:text-zinc-300">{tag.name}</h3>
                      </>
                    ) : (
                      <h3 className="text-sm font-bold uppercase tracking-widest text-zinc-400">Untagged</h3>
                    )}
                    ...
                    {items.map(task => (
                      <div key={task.id} className="group relative flex flex-col gap-3 p-4 bg-white border border-zinc-200 dark:bg-zinc-900/50 dark:border-zinc-800 rounded-xl hover:border-brand-navy/50 hover:shadow-md transition-all">
                        ...
                        <div className="flex items-center justify-between mt-1 pt-3 border-t border-zinc-100 dark:border-zinc-800">
                          <button
                            onClick={() => scheduleTaskToDay(task, dateKey)}
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-brand-navy/10 text-brand-navy dark:bg-brand-navy/20 dark:text-brand-navy hover:bg-brand-navy hover:text-white text-xs font-bold rounded-md transition-colors shadow-sm"
                          >
                            <ArrowRightToLine className="w-3.5 h-3.5" />
                            Plan for {new Date(dateKey + 'T00:00:00').toLocaleDateString(undefined, { weekday: 'short' })}
                          </button>
                          <button
                            onClick={() => archiveMasterTask(task.id)}
                            className="flex items-center gap-1.5 px-3 py-1.5 text-zinc-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/30 text-xs font-bold rounded-md transition-colors"
                          >
      ...
      {isBankOpen && (
        <div
          className="fixed inset-0 bg-black/20 dark:bg-black/60 z-40 backdrop-blur-sm transition-opacity"
        ...
        <div className="fixed inset-0 bg-black/40 dark:bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 w-full max-w-lg rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-5 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50">
              ...
              <button onClick={() => { setIsModalOpen(false); resetModal(); }} className="text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <form onSubmit={handleAddTaskSubmit} className="flex flex-col gap-5 p-6">
              ...
              <div className="flex gap-3 items-start">
                <button
                  ...
                  className="p-1 mt-0.5 hover:scale-110 transition-transform shrink-0"
                ...
                <div className="flex-1 flex flex-col">
                  <input
                    ...
                    autoFocus
                    className="w-full text-xl font-semibold bg-transparent border-0 border-b-2 border-transparent focus:border-brand-navy dark:focus:border-brand-navy focus:ring-0 px-1 pb-1 transition-colors text-zinc-900 dark:text-white placeholder:text-zinc-400"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-3 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-2 col-span-2 sm:col-span-1 focus-within:ring-2 ring-brand-navy/20 relative">
                  <div className="flex items-center gap-2 h-full relative">
                    {/* Tag Dot / Color Picker Trigger */}
                    {(newTaskTagId || isCreatingTag) ? (
                      <button
                        type="button"
                        onClick={() => setIsPaletteOpen(!isPaletteOpen)}
                        className="w-4 h-4 rounded-full shrink-0 shadow-sm border border-black/10 transition-transform hover:scale-110"
                        style={{ backgroundColor: tempTagColor }}
                      ...
                      <div className="absolute top-full left-0 mt-2 p-2 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg shadow-xl z-50 flex gap-2 w-max animate-in fade-in zoom-in-95 duration-100">
                        {aestheticColors.map(color => (
                          <button
                            key={color}
                            type="button"
                            className={cn("w-6 h-6 rounded-full hover:scale-110 transition-transform", tempTagColor === color && "ring-2 ring-offset-2 ring-brand-navy dark:ring-offset-zinc-800")}
                            style={{ backgroundColor: color }}
                            onClick={() => handleUpdateTagColor(color)}
                          />
                        ))}
                        {/* Custom Color Native input fallback */}
                        <div className="relative w-6 h-6 rounded-full overflow-hidden shrink-0 hover:scale-110 transition-transform border border-zinc-200 dark:border-zinc-700 bg-zinc-100 flex items-center justify-center">
                          ...
                          <input
                            type="color"
                            value={tempTagColor || "#000000"}
                            onChange={e => handleUpdateTagColor(e.target.value)}
                            className="w-10 h-10 absolute -top-2 -left-2 cursor-pointer opacity-0"
                          />
                        </div>
                      </div>
                    )}
                    ...
                    {!isCreatingTag ? (
                      <select
                        ...
                        onChange={e => {
                          if (e.target.value === "NEW") {
                            ...
                            setNewTaskTagId(e.target.value);
                            setIsPaletteOpen(false);
                          }
                        }}
                        className="bg-transparent border-none text-sm focus:ring-0 w-full text-zinc-800 dark:text-zinc-200 cursor-pointer appearance-none p-0 outline-none"
                      >
                      ...
                      <div className="flex items-center gap-2 w-full h-full">
                        <input
                          ...
                          className="bg-transparent border-none text-sm focus:ring-0 w-full p-0 h-auto outline-none text-zinc-900 dark:text-zinc-100"
                          autoFocus
                        />
                        <button type="button" onClick={() => { setIsCreatingTag(false); setIsPaletteOpen(false); }} className="text-zinc-400 hover:text-red-500 transition-colors">
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                    )}
              ...
              <div className="flex items-start gap-2 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-3 focus-within:ring-2 ring-brand-navy/20">
                ...
                <textarea
                  ...
                  value={newTaskNotes || ""} onChange={e => setNewTaskNotes(e.target.value)}
                  className="bg-transparent border-none text-sm outline-none w-full resize-none text-zinc-800 dark:text-zinc-200 placeholder:text-zinc-400"
                />
              </div>
      ...
      <button
        ...
        className="fixed bottom-8 right-8 w-14 h-14 bg-brand-navy text-white rounded-full shadow-2xl hover:bg-brand-navy/90 hover:scale-105 active:scale-95 transition-all flex items-center justify-center z-40 group"
      >
        <Plus className="w-6 h-6 group-hover:rotate-90 transition-transform duration-300" />
      </button>

      {/* Main App */}
      <main className="flex w-full max-w-5xl flex-col bg-white shadow-2xl dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden relative z-10 transition-transform duration-300 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 sticky top-0 z-20">
          <button
            onClick={goToToday}
            className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-brand-navy dark:text-zinc-300 bg-brand-navy/5 hover:bg-brand-navy/10 dark:bg-zinc-900 dark:hover:bg-zinc-800 rounded-full transition-colors"
          >
          ...
          <div className="flex items-center gap-4">
            <button onClick={() => shiftDate(-1)} className="p-2 text-zinc-500 hover:text-brand-navy hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-full transition-all">
              <ChevronLeft className="w-5 h-5" />
            </button>
            <span className="text-lg font-bold text-zinc-800 dark:text-zinc-200 select-none min-w-[220px] text-center">
              {formattedDate}
            </span>
            <button onClick={() => shiftDate(1)} className="p-2 text-zinc-500 hover:text-brand-navy hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-full transition-all">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-brand-sage bg-brand-sage/10 hover:bg-brand-sage/20 rounded-full transition-colors"
          >
          ...
          <div className="hidden sm:flex items-center gap-6 bg-white dark:bg-zinc-950 p-4 rounded-xl shadow-sm border border-zinc-100 dark:border-zinc-800 relative z-0 origin-right">
            <div className="flex flex-col text-sm font-medium gap-3">
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-full bg-brand-navy"></span>
                <span className="text-zinc-600 dark:text-zinc-400">Tasks: {tasksCompleted}/{Math.max(1, tasksArray.length)}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-full bg-brand-sage"></span>
                <span className="text-zinc-600 dark:text-zinc-400">Prior: {prioritiesCompleted}/{Math.max(1, prioritiesArray.length)}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-full bg-[#475569]"></span>
                <span className="text-zinc-600 dark:text-zinc-400">Goals: {goalsCompleted}/{Math.max(1, goalsArray.length)}</span>
              </div>
            </div>
            <ConcentricRings
              outerTotal={Math.max(1, tasksArray.length)}
              outerCompleted={tasksCompleted}
              innerTotal={Math.max(1, prioritiesArray.length)}
              innerCompleted={prioritiesCompleted}
              centerTotal={Math.max(1, goalsArray.length)}
              centerCompleted={goalsCompleted}
        ...
        <div className="flex flex-col md:flex-row flex-1 shrink-0">
          ...
          <div className="flex-[1.2] border-r-2 border-zinc-200 dark:border-zinc-800 p-6 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-4 flex-1">
                ...
                <div key={task.id} className="flex items-start gap-3 group border-b border-zinc-100 dark:border-zinc-800/50 pb-3 isolate">
                  <div className="relative flex items-center justify-center mt-0.5 shrink-0">
                    <input
                      ...
                      onChange={() => toggleDayTaskDone(task.id)}
                      className="peer appearance-none w-5 h-5 border-2 border-brand-navy/30 rounded-md checked:bg-brand-navy checked:border-brand-navy transition-all cursor-pointer"
                    />
                  ...
                  <div className="flex-1 flex flex-col">
                    <span className={cn(
                      "text-foreground text-lg transition-all duration-200 select-none leading-tight mt-0.5 cursor-pointer",
                      task.is_done && "line-through text-zinc-400 dark:text-zinc-600"
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => toggleDayTaskPriority(task.id)} className="p-1.5 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-md transition-colors" title="Make Priority">
                      <Star className="w-4 h-4 text-zinc-400 hover:text-brand-sage" />
                    </button>
                    <button onClick={() => removeDayTask(task.id, task.master_id)} className="p-1.5 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-md transition-colors" title="Delete from day">
              ...
              {/* Empty rows to look like ruled paper */}
            ...
            {/* Meals block at bottom of To Do's column */}
          ...
          <div className="flex-1 flex flex-col border-none">
              ...
              <div className="flex flex-col gap-0">
                {prioritiesArray.map((task, idx) => (
                  <div key={task.id} className="flex items-center gap-3 group border-b border-zinc-200 dark:border-zinc-800 py-3 isolate">
                    <span className="font-mono text-lg font-bold text-brand-sage/60 w-6 shrink-0 text-center">
                      {idx + 1}
                    </span>
                    <div className="flex-1 flex flex-col cursor-pointer overflow-hidden" onClick={() => toggleDayTaskDone(task.id)}>
                      <span className={cn(
                        "text-foreground text-lg transition-all duration-200 select-none leading-tight truncate",
                        task.is_done && "line-through text-zinc-400 dark:text-zinc-600"
                      )}>
                        {task.text}
                      </span>
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => toggleDayTaskPriority(task.id)} className="p-1.5 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-md transition-colors" title="Remove Priority">
                        <Star className="w-4 h-4 text-brand-sage fill-brand-sage" />
                      </button>
                      <button onClick={() => removeDayTask(task.id, task.master_id)} className="p-1.5 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-md transition-colors shrink-0" title="Delete from day">
                        <Trash2 className="w-4 h-4 text-zinc-400 hover:text-red-500" />
                      </button>
                    </div>
                    ...
                    <div className="relative flex items-center justify-center shrink-0 ml-2">
                      <input
                        ...
                        checked={task.is_done}
                        onChange={() => toggleDayTaskDone(task.id)}
                        className="peer appearance-none w-5 h-5 border-2 border-brand-sage/40 rounded-full checked:bg-brand-sage checked:border-brand-sage transition-all cursor-pointer"
                      />
                ...
                {Array.from({ length: Math.max(0, 5 - prioritiesArray.length) }).map((_, i) => (
                  <div key={`empty-prior-${i}`} className="flex items-center gap-3 border-b border-zinc-200 dark:border-zinc-800 py-3">
                    <span className="font-mono text-lg font-bold text-brand-sage/30 w-6 shrink-0 text-center">
                      {prioritiesArray.length + i + 1}
                    </span>
                    ...
                    <div key={task.id} className="flex items-center gap-3 group isolate">
                      <div className="relative flex items-center justify-center shrink-0">
                        <input
                          ...
                          checked={task.is_done}
                          onChange={() => toggleDayTaskDone(task.id)}
                          className="peer appearance-none w-4 h-4 border-2 border-zinc-300 dark:border-zinc-700 rounded-sm checked:bg-zinc-400 checked:border-zinc-400 transition-all cursor-pointer"
                        />
                      ...
                      <span className={cn("text-base transition-all flex-1 cursor-pointer", task.is_done ? "line-through text-zinc-400" : "text-zinc-700 dark:text-zinc-300")} onClick={() => toggleDayTaskDone(task.id)}>
                        {task.text}
                      </span>
                      <button onClick={() => removeDayTask(task.id, task.master_id)} className="opacity-0 group-hover:opacity-100 p-1.5 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-md transition-all">
                        <Trash2 className="w-4 h-4 text-zinc-400 hover:text-red-500" />
                      </button>
                    </div>
                  ))
            ...
            <div className="p-6 border-t-2 border-zinc-200 dark:border-zinc-800 flex flex-col gap-4">
              <h3 className="text-md font-bold text-zinc-600 dark:text-zinc-400 italic mb-2 uppercase tracking-wider">
                Health & Habits:
              </h3>
              ...
              <div className="flex flex-col gap-5">
                ...
                <div className="flex items-center gap-4 group">
                  ...
                  <div className="flex-1 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      {dayData.water === 8 && (
                        <div className="flex items-center gap-1 text-[10px] font-bold text-blue-500 uppercase tracking-widest bg-blue-50 dark:bg-blue-900/30 px-2 py-0.5 rounded-full border border-blue-200 dark:border-blue-800 animate-in fade-in zoom-in duration-300">
                ...
                {/* Steps Counter */}
                <div className="flex items-center gap-4 group">
                  <div className="flex items-center justify-center w-8 h-8 rounded-full bg-orange-50 dark:bg-orange-900/30 text-orange-500 shrink-0 relative">
                  ...
                  <div className="flex-1 flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800/80 pb-1">
                    <div className="flex items-center gap-2">
                      ...
                      {(parseInt(dayData.steps || "0") >= parseInt(dayData.step_goal || "10000")) && (
                        <div className="flex items-center gap-1 text-[10px] font-bold text-[#475569] uppercase tracking-widest bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full border border-slate-300 dark:border-slate-600 animate-in fade-in zoom-in duration-300">
                          <CheckCircle2 className="w-3 h-3 text-brand-sage" /> Goal Met
                        </div>
                      )}
                    </div>
        ...
        <div className="border-t-4 border-brand-navy flex flex-col bg-zinc-50 dark:bg-zinc-900/30 shrink-0">
          ...
          <div className="flex flex-col flex-1 min-h-[150px]">
            <div className="p-6 pb-2 border-b-2 border-zinc-200 dark:border-zinc-800 flex items-center justify-between bg-zinc-50 dark:bg-zinc-900 group">
              ...
              <div className="flex items-center gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                <button
                  ...
                  className="p-1.5 text-zinc-500 hover:text-brand-navy hover:bg-zinc-200 dark:hover:bg-zinc-800 rounded-md transition-colors"
                  title="Insert Bullet"
                >
                  <List className="w-4 h-4" />
                </button>
                <button
                  onClick={() => insertNoteFormatting("1. ")}
                  className="p-1.5 text-zinc-500 hover:text-brand-navy hover:bg-zinc-200 dark:hover:bg-zinc-800 rounded-md transition-colors"
            ...
            <textarea
              ref={notesRef}
              className="w-full flex-1 p-6 bg-transparent resize-none outline-none text-zinc-800 dark:text-zinc-200 placeholder:text-zinc-400 font-serif leading-relaxed line-clamp-none border-none focus:ring-0"
              placeholder="Jot down notes, meeting minutes, or brilliant ideas here..."
              value={dayData.notes || ""}
